<template>
  <div class="comment-item">
    <div class="comment-content" :class="{'comment-highlight':isReplying}">
      <!-- 自定义评论组件 -->
...
// 定义点击回复事件，传递回复目标的评论对象
const emit = defineEmits<{
  (e: 'clickReply', comment: VideoCommentsVO): void
}>();
// 事件处理函数
const isReplying = ref(false);

const onClickReply = () => {
  emit('clickReply', props.commentObject); // 把评论对象返回
}
